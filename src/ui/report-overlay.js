'use strict';
/* ---------- in-game board report and decision flow ---------- */
var REPORT = {
  page:0,
  locked:false,
  outcome:null,
...
function _reportMetricDelta(metric){
  var d=metric.cur-metric.last, tone, disp;
  if(Math.abs(d)<0.05){
    return {tone:'flat', disp:'-'};
  }
  tone=((d>0)===metric.goodUp)?'good':'bad';
  disp=metric.money ? metricDeltaText(metric, d) : metricDeltaText(metric, d);
...
function _showDecisionOutcome(outcome){
...
function _chooseReportOption(index){
  ...
  REPORT.outcome=resolveOutcome(REPORT.data.quarter, option, GAME.stats);
  REPORT.locked=true;
  applyBoardDecision(REPORT.outcome);
  _showDecisionOutcome(REPORT.outcome);
  $('repDecision').textContent='BOARD DECISION: '+option.title.toUpperCase();
...
function goToNextReportQuarter(){
  try{
    ...
    var nextId=getNextQuarterId(REPORT.data.quarter.id);
    if(!nextId){
    ...
    GAME.playbackOutcome=REPORT.outcome;
    setTimelineForCurrentQuarter(REPORT.outcome);
    closeReport();
    seekSimulation(0);
    paused=false;
    setScene('simulation');
